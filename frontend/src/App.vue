...
<template>
  <div class="min-h-full flex flex-col">
    <header class="bg-slate-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-10">
      <div class="font-semibold flex items-center gap-2">🏠 语音仓储管家</div>
      <nav class="flex gap-1 text-sm">
        <button v-for="t in tabs" :key="t.id"
          :class="['px-3 py-1 rounded', tab===t.id ? 'bg-white text-slate-900' : 'hover:bg-slate-800']"
          @click="tab=t.id">{{ t.icon }} {{ t.label }}</button>
      </nav>
    </header>
